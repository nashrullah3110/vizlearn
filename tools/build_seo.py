...
AUTHOR_LD = {
    ...
    "sameAs": [
        ...
        "https://www.kaggle.com/ashishjangra27",
    ],
}
...
def generated_description(rel, mods):
    ...
    if is_topic_page(rel):
        ...
        return "%s %d free interactive modules, in teaching order - no login, no setup." % (
            t["lead"], n)
    if is_static_page(rel):
        # The copy carries the same %(modules)d / %(tracks)d placeholders the
        # page body does, and build_static_pages.py fills them there. This
        # path did not, so about.html shipped a literal "%(modules)d" in its
        # description, its og: and twitter: cards and its JSON-LD - the one
        # page a reviewer opens to decide whether the site is finished.
        return STATIC_COPY[rel]["description"] % counts()
    if is_tool_page(rel):

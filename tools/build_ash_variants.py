...
BASE_CSS = """
    ...
    /* The web itself is decoration. Without this the spoke lines sit on top
     * of the link's own hit area and swallow the pointer partway along a
     * spoke, so only the dot and the label were reliably hoverable. */
    .vzx-r-ring, .vzx-r-spoke, .vzx-r-area, .vzx-r-scale { pointer-events: none; }
    ...
    /* The lead is both the hit target and the highlight: a wide transparent
     * stroke by default, so pointing anywhere along a spoke picks that track
     * rather than only the dot at its end, and a thin accent stroke once the
     * node is on. */
    .vzx-r-lead {
      stroke: transparent;
      stroke-width: 18;
    ...
    .vzx-r-node.is-on .vzx-r-lead { stroke: var(--accent-primary); stroke-width: 2; }

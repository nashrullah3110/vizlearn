#!/usr/bin/env python3
"""Render the scikit-learn modules from tools/sklearn_topics.py.
...
Every editor carries data-vz-packages="scikit-learn". The blocks on a page
share one interpreter, so the 88MB of wheels - scikit-learn, scipy, numpy and
openblas - download once per page however many steps there are, and the import
cost is paid by the prelude while the runner is still showing its loading
state.

The article text is written to content/articles/sklearn/ where
build_articles.py picks it up, and the questions reach build_labs.py through
tools/labs.py.

    python3 tools/build_sklearn_topics.py
"""
...
from sklearn_topics import TOPICS
...
def editor(filename, code, wheels=(), prelude=""):
    ...
    if prelude:
        pre = ('<script type="text/plain" class="py-prelude">%s</script>\n                    '
               % prelude.strip())
    return """                <div class="vz-py" data-vz-py data-vz-packages="scikit-learn"%(extra)s data-vz-label="scikit-learn">
                    %(pre)s<script type="text/plain" class="py-src">%(code)s</script>
                    <div class="vz-code-bar">
                        <span class="vz-code-dot"></span><span>%(file)s</span>
                        <span class="vz-code-lang">scikit-learn</span>
                    </div>
                    <div class="vz-code" data-vz-code="python">
                        ...
                        <div class="vz-code-scroll">
                            ...
                            <textarea class="vz-code-input py-editor" aria-label="scikit-learn code editor"
...
def main():
    ...
    print("sklearn pages written  : %d" % len(TOPICS))

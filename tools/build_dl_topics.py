#!/usr/bin/env python3
"""Render the generated deep learning modules from tools/dl_topics.py.

The track covered training mechanics thoroughly - gradients, normalisation,
regularisation, schedules - and stopped at a dense network. Autoencoders,
GANs, diffusion, contrastive learning and beam search had no page anywhere on
the site, while the words were used across it freely.

As on the other generated tracks the arithmetic runs in the browser
(assets/vizlearn-dl.js): the autoencoder really reconstructs from its
bottleneck, the diffusion page really applies the forward schedule and really
inverts it, and beam search really searches the beam.

Registers its own catalog entries, so a new topic needs one edit -
dl_topics.py - plus its place in sequence.py.

    python3 tools/build_dl_topics.py
"""
...
CSS = """
        ...
        .dl-note code { color: var(--accent-primary); }
        .vz-dl-lead { margin-top: 0.5rem; max-width: 60ch; color: var(--text-muted); }
"""
...
def viz(t):
    ...
    return """                <div class="vz-dl" data-vz-dl>
                    <script type="application/json" class="ml-config">%s</script>
                    <p class="dl-fallback">This module needs JavaScript: the
...
def page(t):
    ...
    notes = "\n".join('                        <div class="dl-note">%s</div>' % n
                      for n in t["notes"])
...
def main():
    ...
    print("dl pages written : %d" % len(TOPICS))

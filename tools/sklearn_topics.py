...
2. <strong>Swap the model.</strong> Change `LogisticRegression` to `RandomForestClassifier` in step three and re-run everything after it. Nothing else needs to change.
3. <strong>Remove the indicator.</strong> Drop `add_indicator=True` and compare the cross-validated average precision.
4. <strong>Break the order.</strong> Fit the imputer on all of `X` before the split, and watch the test score improve for the wrong reason.

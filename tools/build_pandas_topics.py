#!/usr/bin/env python3
"""Render the NumPy modules from tools/pandas_topics.py.
...
Every editor carries data-vz-packages="pandas". The blocks on a page share
one interpreter, so the library downloads once however many steps there are.

The article text is written to content/articles/pandas/ where
build_articles.py picks it up, and the questions reach build_labs.py through
tools/labs.py.

    python3 tools/build_pandas_topics.py
"""
...
from pandas_topics import TOPICS
...
def editor(filename, code, wheels=(), prelude=""):
    ...
    if prelude:
        pre = ('<script type="text/plain" class="py-prelude">%s</script>\n                    '
               % prelude.strip())
    return """                <div class="vz-py" data-vz-py data-vz-packages="pandas"%(extra)s data-vz-label="pandas">
...
def main():
    ...
    print("pandas pages written    : %d" % len(TOPICS))

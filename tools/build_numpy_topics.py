#!/usr/bin/env python3
"""Render the NumPy modules from tools/numpy_topics.py.
...
The article text is written to content/articles/numpy/ where
build_articles.py picks it up, and the questions reach build_labs.py through
tools/labs.py.

    python3 tools/build_numpy_topics.py
"""
...
from numpy_topics import TOPICS
...
def editor(filename, code, wheels=(), prelude=""):
    ...
    if prelude:
        pre = ('<script type="text/plain" class="py-prelude">%s</script>\n                    '
               % prelude.strip())
    return """                <div class="vz-py" data-vz-py data-vz-packages="numpy"%(extra)s data-vz-label="NumPy">
...
def main():
    ...
    print("numpy pages written    : %d" % len(TOPICS))
